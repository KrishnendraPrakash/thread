...
See pyproject.toml and docs/development.md for the established Python 3.11+ packaging, Ruff checks, and CLI smoke commands. Behavior test directories exist but contain no tests yet; a zero-test discovery run is not an acceptance pass. The agent/model-run command is not implemented. Discover current configuration before claiming a command exists or has passed.
