...
These TOML files reserve configuration shapes for local, hosted, and private profiles. They parse as TOML but the current CLI does not load or enforce them. They are not finalized configuration contracts.
...
The experimental local field workflow uses explicit CLI flags and loopback Ollama rather than these draft files. See [working setup](../docs/first-workflow.md).
