...
- Python setup MUST probe the exact executable entered by the user before persisting it, independently of local-model availability. The setup model check MUST use that same executable without rereading settings. Python failure MUST preserve the previous setting; an unavailable Ollama server MUST NOT discard a successfully saved Python setting. Relative interpreter paths MUST be rejected rather than resolved against the extension installation.
