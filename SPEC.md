...
The user explicitly requested VS Code as the first developer interface for repository understanding, debugging assistance, feature writing, and document summaries. This authorizes the editor/backend implementation and bounded approved edits. The subsequent user request authorizes preparation and publication of a clearly labeled public preview. It does not declare the complete M0–M6 gates passed or select a license. The later request to reduce developer setup authorizes a managed installer; each actual runtime/model download in the product still requires its own explicit confirmation.

- The extension MUST use a selected installed local Ollama completion model, an automatically detected Python 3.9+ interpreter (with an optional explicit user override), and a scoped subprocess protocol. Repository configuration cannot choose the executable. No paid credential, public HTTP server, or automatic provider fallback is required. Optional managed local setup MAY download a runtime and model only after explicit user confirmation.
- Normal Python setup MUST discover and probe compatible installed interpreters without asking for a path. Discovery MUST be bounded and cancellable, exclude relative PATH entries and executables within the inspected workspace (including symlink aliases), and never download a runtime. A stale user override MAY fall back to a detected interpreter with a visible notice. The advanced override MUST probe the exact executable entered by the user before persisting it, independently of local-model availability. The setup model check MUST use that same executable without rereading settings. Python failure MUST preserve the previous setting; an unavailable Ollama server MUST NOT discard a successfully saved Python setting. Relative interpreter paths MUST be rejected rather than resolved against the extension installation.
- Managed local setup MUST disclose runtime/model identity, download sizes, destinations and model terms before confirmation. It MUST use a pinned official runtime with a verified checksum, extract into extension-owned storage without archive traversal, avoid system package managers/admin scripts, and support cancellation and clear recovery. Runtime/model downloads MUST NOT be triggered by retrieved text or model output. The initial managed installer targets Apple Silicon macOS 14+; other supported systems retain existing Ollama setup.
- A managed model server MUST bind only to literal IPv4 loopback on a private port, disable its cloud mode, and store models separately from the user’s existing installation. The extension owns and stops its server, never an unrelated server. Backend endpoint validation MUST reject remote hosts. Managed setup changes initialization, not the evidence, approval or answer-verification guarantees.
...
- The editor generation schema MUST require 1–12 cited claims, matching independent response validation. Instructions MUST allow explicit uncertainty about missing context without inventing facts to fill the count. Invalid claim counts MUST be rejected with an actionable error; schema compliance does not establish factual support.
