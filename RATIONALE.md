...
## Managed local runtime — 2026-09-25

The user reported that automatic Python setup still left another developer blocked by missing Ollama and requested less developer-side configuration. The extension now offers an explicitly confirmed managed runtime/model setup on Apple Silicon macOS 14+, while preserving an existing local Ollama option. This transfers installation work into the extension without removing the need for compute, disk space or an initial download. It is not an implicit hosted fallback.

Ollama 0.34.4’s official standalone macOS archive is pinned by SHA-256, installed under extension storage, and started on a private loopback port with cloud disabled. No Homebrew, Docker, sudo or remotely supplied install script is executed. Starter choices are qwen2.5-coder:1.5b (about 986 MB) and :7b (about 4.7 GB), both listed with Apache 2.0 terms. The smaller model is recommended for download size, not certified accuracy. Model registry tags may change; Ollama verifies downloaded layers and the analysis trace records installed artifact metadata. Model-download network endpoints are distinct from local inference.

Official references: [macOS requirements](https://docs.ollama.com/macos), [runtime release](https://github.com/ollama/ollama/releases/tag/v0.34.4), [small model](https://ollama.com/library/qwen2.5-coder:1.5b), [larger model](https://ollama.com/library/qwen2.5-coder:7b), [pull API](https://docs.ollama.com/api/pull). Full model-download/inference trials on clean machines and non-Apple-Silicon installers remain future validation, not established results.
