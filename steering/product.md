...
The first application is one user per installation, with a Python core, terminal interface, SQLite persistence, and scoped local tools. Keep the design understandable and portable; publish support claims only for configurations actually tested. A public repository does not imply a hosted multi-tenant service.
...
- Provisional first tasks: answer questions about local project files, remember/correct project decisions, and perform approved bounded file edits. The user has not yet finalized the three real priority tasks.
